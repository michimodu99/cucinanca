...
The dispensa places the display and lede in columns 1 to 5 — since 12/09/2026 on a dark panel that bleeds to the top and left window edges, fills the first viewport (`100dvh − topbar − 96px`, `align-self: start` so it never stretches with the index) and carries a muted, looping cooking clip (`video/hero.mp4`, poster `img/hero-poster.webp`) under a black veil (0.7 bottom → 0.3 top); display, lede and pantry line are paper on it, the pantry rule is white at 35%. It is the one deliberate exception to black-on-white, chosen for the landing; `prefers-reduced-motion` swaps the clip for the poster. On mobile the panel bleeds on both sides at 60dvh. The input, chips and ingredient index sit in columns 7 to 12, leaving column 6 as air; the fixed bottom action bar sits on the same grid, count in columns 1 to 6, black button in 7 to 12. The results page puts the headline in columns 1 to 8 and the count in 9 to 12, then a sticky filter bar under the sticky 56px topbar, then full-bleed ruled rows (96px photo, body, coverage) whose hover wash extends to the viewport edge. The recipe reader fills the viewport below the topbar (`100dvh - topbar`): desktop is a two-page spread split by a hairline, with the cover page holding a 62%-wide 4:5 plate beside the title; mobile is a horizontal scroll-snap strip of full-width pages (cover, ingredients, one page per step) with a 56px nav bar respecting the safe-area inset.
...
### Named Rules
**The Paper Only Rule.** Every surface is paper (one exception: the dispensa hero panel, dark with the cooking clip, see Layout). Layering is signaled by a rule, never by a shadow or a tint shift; the only non-paper fill is mist inside a field, a frame or a hover.
