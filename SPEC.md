...
Versione 0.1 · 11 settembre 2026 · stato: **in revisione**
...
| Persone | 4, nessuna intolleranza; cena in casa, pranzo in mensa |
| Esclusioni fisse | piselli, fagiolini, carciofi, pomodori (e derivati: passata, pelati, concentrato, ketchup), broccoli, cavolfiore |
| Attrezzatura posseduta | piano a induzione, forno, microonde, 3 padelle, set coltelli, bilancia precisa, taglieri |
| Attrezzatura NON posseduta | frullatore/minipimer (ricette che lo richiedono sono incluse ma segnalate) |
...
| Dosi | "Dosi per: 4 persone" | "Porzioni" | ✓ default 4 |
| Costo | Molto basso · Basso · Medio · Alto · Molto alto | — | 3 fasce + **stima in € per 4 persone a prezzi PT** |
...
- `reinventata`: come una ricetta classica è stata adattata alle esclusioni (es. cacciatora in bianco).
...
- Sinistra: foto a piena altezza, titolo, descrizione, riga meta (difficoltà · prep · cottura · riposo · porzioni · costo), badge attrezzatura, nota `reinventata` se presente.
- Destra: ingredienti (con quantità per 4; evidenziati i mancanti; nota PT dove serve) e procedimento numerato. Se il procedimento eccede l'altezza, continua nella spread successiva.
...
### 4.4 Fuori scope v1
Timer, porzioni variabili, persistenza dispensa, preferiti, note personali, valori nutrizionali completi, API esterne, PWA offline (valutabile in v2: è quasi gratis con un service worker).
...
```jsonc
{
  ...
  "porzioni": 4,
  "costo": { "fascia": "basso", "stima_eur": 6 },                 // basso ≤ 8 € · medio 8–15 € · alto > 15 € (per 4, prezzi PT)
  "attrezzatura": ["pentola", "padella"],   // padella | pentola | forno | microonde | teglia | frullatore | mattarello | stampo | frusta
  "tag": ["autunno", "one-pan", "avanzi"],  // + veloce | low-cost | da-ospiti | inverno | classico | portoghese
  ...
  "ingredienti": [
    { "id": "riso-carnaroli", "qta": 320, "unita": "g", "note": "", "opzionale": false,
...
Regole:
- `ingredienti[].id` deve esistere in `ingredienti.json`; nessun id con `vietato: true` è ammesso (il validatore fallisce).
...
```jsonc
{ "id": "pomodoro", "nome": "Pomodoro", "alias": ["pomodori", "pomodorini", "passata", "pelati", "concentrato di pomodoro", "ketchup"],
...
- **Dispensa base** (assunzione da confermare in DOMANDE.md): olio evo, sale, pepe, aglio, cipolla, burro, farina 00, zucchero, aceto, limone, dado/brodo, parmigiano, peperoncino, alloro, rosmarino secco.
...
- Modello: **FLUX Schnell** (≈ 0,003 $/immagine) per il primo passaggio; **FLUX Dev** (≈ 0,025 $) per rigenerare quelle non convincenti. Budget atteso < 1 $ per 50 ricette.
...
- Sito statico, **HTML/CSS/JS vanilla, senza build**. Hosting **GitHub Pages** (repo `michimodu99/cucina` o simile; nome da decidere).
...
| 1 | Tassonomia, schema, 50 schede, validatore, match.js con test | validate + test verdi |
