...
- **Video nella hero della dispensa**: pannello scuro a sinistra, clip in loop muta con velo e testo bianco; anche su mobile; poster fisso con `prefers-reduced-motion`; in pausa quando si cambia vista. `npm run video:optimize` comprime una clip da `video/raw/` (ffmpeg-static, nessuna installazione). In repo c'è la clip `chop` scelta da Michele (taglio di un ortaggio, 13″, verticale 720×1280, 1,4 MB); `frigo` (2″) è in `video/raw/` ma troppo corta per un loop. Flusso nel README, "Video di sfondo".
